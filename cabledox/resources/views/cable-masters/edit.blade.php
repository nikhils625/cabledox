...
@section('title', 'Edit Cable Master')
...
<div class="row clearfix">
    <div class="col-md-12">
        <div class="card">  
            ...
            <div class="header"><h2>{{ __('Edit Cable Master') }}</h2></div>
            <div class="body">
                <form method="POST" action="{{ route('cable-masters.update', @$cableMaster->id) }}" data-parsley-validate novalidate> 
                    @csrf
                    @method('PATCH')
                    <div class="form-group row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input type="text" name="cable_type_id" id="cable_type_id" class="form-control @error('cable_type_id') is-invalid @enderror" value="{{ $cableMaster->cable_type_id }}" required autocomplete="cable_type_id" placeholder="Please enter cable type">
                                    @error('cable_type_id')
                        ...
                        <div class="col-md-6">
                            <div class="form-group row">
                                ...
                                <div class="col-md-7">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <input type="number" name="cores" id="cores" class="form-control @error('cores') is-invalid @enderror" value="{{@$cableMaster->cores ?? 1}}" required autocomplete="cores" placeholder="Enter cores" min="1">
                                            @error('cores')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                        ...
                                        <div class="col-md-2 text-center">
                                            <label for="cores" class="col-form-label">
                                                <i class="fa fa-times"></i>
                                            </label>
                                        </div>
                                        <div class="col-md-4">
                                            <input type="number" name="no_of_pair_triple_quad" id="no_of_pair_triple_quad" class="form-control @error('no_of_pair_triple_quad') is-invalid @enderror" value="{{ ($cableMaster->no_of_pair_triple_quad > 1)? $cableMaster->no_of_pair_triple_quad : null }}" min="0" autocomplete="no_of_pair_triple_quad">
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            <label for="no_of_pair_triple_quad" class="col-md-2 col-form-label text-md-left">{{ __('Core Details:') }}<span class="text-danger"> *</span></label>
                            <div class="col-md-12">
                                        ...
                                        <tbody class="htmlToAppend">
                                            @if(!$cableMaster->cableMasterCoreDetails->isEmpty())
                                                @foreach($cableMaster->cableMasterCoreDetails as $key => $detail)
                                                    <tr class="row_core_details core_{{$detail->core_index}} wire_{{$detail->wire_index}} tr_data_{{$detail->core_index}}_{{$detail->wire_index}}" data-cores="{{$detail->core_index}}" data-wires="{{$detail->wire_index}}">
                                                        <td>Core Name/Colour</td>
                                                        <td><input type="text" name="core_name[{{$detail->core_index}}][{{$detail->wire_index}}]" id="core_name_{{$detail->core_index}}_{{$detail->wire_index}}" class="form-control core_name" value="{{$detail->core_name}}" data-core_index="{{$detail->core_index}}" data-pair_index="{{$detail->wire_index}}" required autocomplete="core_name" placeholder="Please enter core name/colour"></td>
                                                    </tr>
                                                @endforeach
                                            @endif
                                        </tbody>
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.core_name', function() {
            $(this).each(function(k, v) {
                ...
                // $('input[data-pair_index="'+_pairIndex+'"]').val(_thisVal);
